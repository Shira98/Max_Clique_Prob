		...
		if(R[i]==1){
				...
				for(j=color-1; j>=0; j--){
					...
					for(k=0;k<=i-1;k++){
						...
						if(*(c+k)==j){
						 if(*((g+i*n)+k)==1){
						 		no=1;//it cant belong to that class
						}}
					}}
					if(no==1){
						no1=no1+1;
...
//check common array
				for(i=0;i<n;i++)
					common[i]=0;
				for(i=0;i<n;i++){
					if(*((g+index_vertex*n)+i)==1){ //pressent in adj matrix
...
//finding max clique in a graph
int *clique;
clique=(int *)malloc(n*sizeof(int));
