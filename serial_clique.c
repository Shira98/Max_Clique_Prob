...
#include<stdlib.h>
 ...
 int *coloring(int **g,int n,int R[n]){
    int i,j,color=1,j_color,no,k,no1;
    int *c;
...
void MaxClique_vc(int **g,int n,int *c,int R[n], int *Q,int *Qmax){ //returns maxclique in a graph
...
//choose a vertex p with a maximum color C(p) from set R;
        ...
        if(len_clique(n,Q) + *(c+index_vertex) +1 > len_clique(n,Qmax)){
        //Q := Q ∪ {p};
                *(Q+index_vertex)=1;
                ...
                if(len_common!=0){
                    ...
                    // MaxClique(R ∩ Γ(p),C);
                    MaxClique_vc(g,n,color_i,common,Q,Qmax);
                }
                else if(len_clique(n,Q) > len_clique(n,Qmax)){   // else if |Q| > |Qmax| thenQmax := Q;
                    for(i=0;i<n;i++)
                        *(Qmax+i)=*(Q+i);
...
//return Qmax;
 ...
 void maxclique(int **g,int n,int **Qmax){
    int i,j,*c,*clique,R[n],*Q;
    //int *Qmax;
    Q=(int *)malloc(n*sizeof(int));
    //Qmax=(int *)malloc(n*sizeof(int));
    *Qmax = malloc(n * sizeof( int));
    for(i=0;i<n;i++){
        R[i]=1;
        *(Q+i)=0;
        //*(Qmax+i)=0;
